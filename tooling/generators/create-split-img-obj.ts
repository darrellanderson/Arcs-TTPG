...
async function main() {
  ...
  const args = yargs
    .options({
      ...
      p: {
        alias: "preshrink",
        describe: "resize input to be at most P pixels in either dimension",
        type: "number",
        //demand: true,
  ...
  for (let col: number = 0; col < numCols; col++) {
    for (let row: number = 0; row < numRows; row++) {
      ...
      if (fs.existsSync(dstFile)) {
        throw new Error(`Output texture file "${dstFile}" already exists`);
      }
  ...
  if (!fs.existsSync(PATH_UNIT_CUBE_MODEL)) {
    ...
    console.log(`already have "${PATH_UNIT_CUBE_MODEL}", skipping this step`);
  }
