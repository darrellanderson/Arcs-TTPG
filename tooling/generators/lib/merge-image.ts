...
export class MergeImage {
    ...
    async merge(): Promise<MergeImageResult> {
        ...
        for (const srcFileRelativeToPrebuild of this
            ._srcFilesRelativeToPrebuild) {
            ...
            if (cardPixels.w === -1) {
                ...
                const scale =
                    this._preshrink > 0 ? Math.min(scaleW, scaleH) : 1;
        ...
        if (cardImages.length > maxCards) {
            throw new Error(
                `too many cards (${cardImages.length}), max ${maxCards}`
            );
        }
        ...
        const result: MergeImageResult = {
            cols: layout.cols,
            rows: layout.rows,
            imgFileRelativeToAssetsTextures:
                this._dstFileRelativeToAssetsTextures,
        };
        return result;
    }
